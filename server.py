...
def dispatch(method, path, params, body, conn):
    ...
    if m and method == "PUT":
        ...
        if force:
            ...
            conn.execute(
                "UPDATE order_items SET status='R', docking_completed_at=CURRENT_TIMESTAMP WHERE order_id=? AND status IN ('T','C')",
                [oid]
            )
            conn.commit()
            sync_order_status(conn, oid)
    ...
    if method == "GET" and path == "/docking/log":
        ...
        # Order by last action date descending
        # Note: order_items has no updated_at column; use docking_completed_at or created_at
